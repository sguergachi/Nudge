...
public sealed class NudgePredictionHistoryTests
{
    ...
    {
        // This verifies the key invariant: MLDATA followed immediately by SUPPRESS
        // must result in exactly one event (the MLDATA event, now with SuppressReason set),
        // NOT two events (the old bug that caused the zig-zag).
        long now = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
        ...
        // Simulate what HandleSuppress does: try to mutate; only add if mutation fails.
    ...
    public void TryMutateLatest_StandaloneSuppression_NewEventHasSupTriggerSource()
    {
        ...
        // with TriggerSource="sup". This represents a meeting detected before the
        // ML check ran (e.g. user is in Teams continuously).
        long now = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
        var events = new List<MLLiveEvent>();  // no prior MLDATA
    ...
    public void FilterToAiOnly_MixedEvents_RetainsOnlyAiSource()
    {
        var events = new List<MLLiveEvent>
        {
            ...
            new MLLiveEvent { TriggerSource = "int", Score = 0.5 },  // interval fallback
            new MLLiveEvent { TriggerSource = "ai",  Score = 0.2 },
            new MLLiveEvent { TriggerSource = "sup", Score = 0.0 },  // suppression placeholder
    ...
    {
        // Interval-only history (before the ML model is ready) should produce
        // an empty list so the chart shows the "waiting for first AI check" message.
        var events = new List<MLLiveEvent>
        {
    ...
    public void FilterToAiOnly_DoesNotMutateOriginalList()
    {
        ...
        Assert.Equal(2, events.Count);  // original unchanged
    }
    ...
    public void FilterToAiOnly_InterleavedWithAi_ProducesMonotonousAiSequence()
    {
        ...
        // created artificial zig-zags: 0.8 → 0.5 → 0.3 → 0.5 → 0.9 etc.
        // After filtering, only real model predictions remain.
        var events = new List<MLLiveEvent>
        {
            new MLLiveEvent { TriggerSource = "ai",  Score = 0.8 },
            new MLLiveEvent { TriggerSource = "int", Score = 0.5 },
...
{
    // ── IsMeetingTitle only matches title keywords, not bare process names ────
    //
    // Note: a few process names (gotomeeting, bluejeans, whereby) are also in
    // TitleKeywords intentionally — they are uncommon enough that the title
    // alone is sufficient evidence. The test cases below focus on the names
    // that caused the actual double-counting: teams, zoom, slack, etc.

    [Theory]
    [InlineData("teams")]       // MS Teams process name — must NOT match alone
    [InlineData("zoom")]        // Zoom process name (not "zoom meeting") — must NOT match
    [InlineData("slack")]       // Slack process name (not "slack call") — must NOT match
    [InlineData("discord")]     // Discord process name (not "discord voice") — must NOT match
    [InlineData("skype")]       // Skype process name (not "skype for business") — must NOT match
    [InlineData("webex")]       // WebEx process name (not "webex meeting") — must NOT match
    [InlineData("ms-teams")]    // Teams alternate process name — must NOT match
    [InlineData("ringcentral")] // RingCentral process (not "ringcentral meeting")
    [InlineData("lark")]        // Lark process (not "lark meeting")
    [InlineData("dingtalk")]    // DingTalk process (not "dingtalk meeting")
    [InlineData("wemeet")]
    [InlineData("voov meeting")] // Not a title keyword
    [InlineData("tencent meeting")] // Not a title keyword
    public void IsMeetingTitle_BareProcessName_DoesNotMatch(string processName)
    {
        // A window title that is just the process name (app minimised, or the main
        // app window title) should NOT be treated as a meeting title. The process-
        // name check lives in IsMeetingAppRunning() which contributes its own 0.10
        // weight — including it here would double-count.
    ...
    // ── Key separation: process names ≠ title keywords ────────────────────────

    [Fact]
    public void ProcessNames_TeamsZoomSlack_AreNotStandaloneTitleKeywords()
    {
        // The process names most likely to cause double-counting ("teams", "zoom",
        // "slack", "discord") must not appear as standalone title matches.
        // This ensures IsMeetingTitle() and IsMeetingAppRunning() are orthogonal signals.
