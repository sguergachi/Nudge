...
> **Status:** Design / handoff doc for the implementing agent. Targets the `2.0` branch
> (reviewed at v2.0.5, post-UIA-reader). Companion to `EXPERIMENTAL_SIGNAL_MODE.md`.
...
**no nudges**. Quietly scrolling x.com is behaviorally identical to reading documentation
(stable focus, one domain, no media), the synthetic seed has no opinion, predictions never
reach the **85% confidence** trigger (`nudge.cs:94`), and "productive" verdicts keep
resetting the interval timer. Worse, it's a **cold-start deadlock**: nudges are how labels
are created, so no nudges → no labels → the personalization engine never engages.
...
the load site is `nudge.cs:1792` (experimental-only block). Feature build sites
(`nudge.cs:2037-2040`) and label-update flow are untouched.
...
- **Loading:** daemon, experimental block only (`nudge.cs:1792`): parse the TSV into
...
- Verify the 1.9.7 interval fix (low-confidence productive no longer resets the
  interval timer) survives in `2.0` so fallback nudges — the label faucet — keep
  flowing while the model warms up.
...
| `nudge.cs` | load TSV at `:1792`; mode-aware threshold at `:94`; one health log line |
...
5. **Calibration check-ins** (occasional nudge despite "productive" verdicts, to
   keep labels flowing): skip for now — interval fallback already produces labels;
   revisit if label volume stays low.
